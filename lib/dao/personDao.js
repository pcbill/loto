...
// 查詢所有一般獎項（exec_type = 0）中已中獎但尚未領獎的得獎者
var findNormalGameWinnersNotReceived = (callback) => {
    var sl = "SELECT person.uid, person.name, person.table_num, game.gid as game_gid, game.award_list " +
        " FROM person " +
        " INNER JOIN game ON game.id = person.award_game_id " +
        " WHERE game.exec_type = 0 " +
        " AND person.award_time IS NOT NULL " +
        " AND person.getgift_time IS NULL " +
        " AND person.award_game_id < 90000 " +
        " ORDER BY game.gid, person.award_time DESC";
    query(sl, [], callback);
}
exports.findNormalGameWinnersNotReceived = findNormalGameWinnersNotReceived;
...
var cancelReward = (id, callback) => {
    const sl = 'UPDATE person SET award_game_id = 99998, award_time = NOW() WHERE uid = $1';
    const vle = [id];
    query(sl, vle, callback);
}
...
// 查詢非普獎中獎者的報到時間分布（每10分鐘為一區塊）
// 非普獎定義：award_game_id 不是 99999 且不是 99998
var getSpecialWinnerRegistrationDistribution = (callback) => {
    var sl = `
        SELECT 
            to_char(date_trunc('hour', (registration_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Taipei') + 
                   (floor(extract(minute from (registration_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Taipei') / 10) * interval '10 minutes'), 
                   'HH24:MI') as time_slot,
            date_trunc('hour', (registration_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Taipei') + 
                   (floor(extract(minute from (registration_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Taipei') / 10) * interval '10 minutes') as time_slot_raw,
            count(*) as count
        FROM person 
        WHERE registration_time IS NOT NULL 
          AND award_game_id IS NOT NULL
          AND award_game_id NOT IN (99999, 99998)
        GROUP BY time_slot_raw
        ORDER BY time_slot_raw
    `;
    query(sl, [], callback);
}
exports.getSpecialWinnerRegistrationDistribution = getSpecialWinnerRegistrationDistribution;

// 查詢所有報到人員的時間分布（每10分鐘為一區塊）
var getRegistrationDistribution = (callback) => {
    var sl = `
        SELECT 
            to_char(date_trunc('hour', (registration_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Taipei') + 
                   (floor(extract(minute from (registration_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Taipei') / 10) * interval '10 minutes'), 
...
exports.getRegistrationDistribution = getRegistrationDistribution;
