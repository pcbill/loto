...
var getSpecialWinnerRegistrationDistribution = (callback) => {
    var sl = `
        SELECT 
            to_char(date_trunc('hour', registration_time) + 
                   (floor(extract(minute from registration_time) / 10) * interval '10 minutes'), 
                   'HH24:MI') as time_slot,
            date_trunc('hour', registration_time) + 
                   (floor(extract(minute from registration_time) / 10) * interval '10 minutes') as time_slot_raw,
            count(*) as count
        FROM person 
