            ...
            if (result.rows) {
              result.rows.forEach((it)=>{
                ...
                if (it.registration_time) {
                  var regTime = it.registration_time.getTime() + eightHourMinite;
                  it.registration_time = dateFormat(regTime, f);
                }
                if (it.award_time) {
                  var awardTime = it.award_time.getTime() + eightHourMinite;  
                  it.award_time = dateFormat(awardTime, f);
                } 
                if (it.getgift_time) {
                  var getGiftTime = it.getgift_time.getTime() + eightHourMinite;
